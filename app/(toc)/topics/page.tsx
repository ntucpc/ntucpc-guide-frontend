    ...
    return (
        <div className="group bg-white overflow-hidden">
            ...
            <div className="space-y-3">
                {groups.map(group => {
                    const chapterTitle = group.code === "unknown" ? "其他" : `Chapter ${getChapter(group.code).number}`
                    return (
                        <div key={group.code} className="flex flex-col sm:flex-row sm:items-baseline gap-1 sm:gap-4">
                            <div className="flex items-center gap-2 min-w-[90px] shrink-0">
                                ...
                                <span className="text-[11px] font-bold text-gray-400 uppercase tracking-wider">{chapterTitle}</span>
                            </div>
