...
export default function AllTestPage({
    ...
    const sortedArticles = [...articles].sort((a, b) =>
        compKey(getSortKey(a), getSortKey(b))
    )
                            ...
                            {sortedArticles.map((article) => {
                                return (
                                    <TableRow key={article.code}>
                                        ...
                                        <TableCell>
                                            {structure.getArticleChapterTitle(
                                                article.code
                                            )}{" "}
                                            ({article.chapterIndex.join(".")})
                                        </TableCell>
                                        <TableCell>
                                            {structure.getArticleTopicTitle(
                                                article.code
                                            )}{" "}
                                            ({article.topicIndex.join(".")})
                                        </TableCell>
                                        <TableCell>
                                            <HyperRef href={`/${article.code}`}>
