...
import { SidebarCategory, SidebarSection, SidebarGroup, SidebarClientProps } from "./types"

function ScrollSection({ children }: {children: ReactNode}) {
    return <div className={`flex-grow overflow-y-auto mt-2 pr-2
    ...
    return (
        <div className="mb-1">
            ...
            >
                <FontAwesomeIcon 
                    icon={faAngleRight} 
                    className={`text-[10px] text-gray-400 transition-transform duration-200 ${folded ? "" : "rotate-90"}`} 
                />
                <div className="truncate">
                    {group.title}
                </div>
            </div>
            <ul className={`pl-4 space-y-0.5 mt-0.5 border-l border-gray-100 ml-3.5 ${folded ? "hidden" : "block"}`}>
                        ...
                        <li key={item.id} className="select-none">
                            {
                                ...
                                <div className="flex justify-between items-center py-1.5 px-3 text-sm text-gray-300">
                                    ...
                                    <span className="text-[10px] bg-gray-100 text-gray-500 px-1.5 py-0.5 rounded-full">待續</span>
                                </div>
                            }
                        </li>
                    )
...
function SidebarSectionComponent({ section, folding, activeArticle }: 
        { section: SidebarSection, folding: { isFolded: (sid: string, gid: string) => boolean, toggle: (sid: string, gid: string) => void }, activeArticle: string }) {
    return (
...
export function SidebarClient(props: SidebarClientProps) {
    ...
    const [sectionFolded, setSectionFolded] = 
            useState<Map<string, Set<string>>[]>([new Map(), new Map()])

    const isFolded = (cat: number, section: string, group: string) => {
        if (!sectionFolded[cat].has(section)) return defaultFolded
        return sectionFolded[cat].get(section)!.has(group) ? !defaultFolded : defaultFolded
    }
    ...
    return <>
        ...
        <div 
            className={`fixed bg-white shadow-sm w-full z-[999] select-none cursor-pointer border-b 
                ${displaySidebar === true ? "hidden" : (displaySidebar === false ? "block" : "xl:hidden")}`} 
        ...
        <aside 
            className={`fixed bg-white xl:sticky top-16 w-full max-w-[20rem] xl:max-w-none xl:w-80 shrink-0 z-[999] 
                ${displaySidebar === true ? "block" : (displaySidebar === false ? "hidden" : "hidden xl:block")} 
        ...
        >
            ...
            <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-3 text-gray-400">
                    <button onClick={() => updateDefaultFolded(false)} title="全部展開" className="hover:text-indigo-500 transition-colors">
                        <FontAwesomeIcon icon={faExpandArrowsAlt} className="text-sm" />
                    </button>
                    <button onClick={() => updateDefaultFolded(true)} title="全部收合" className="hover:text-indigo-500 transition-colors">
                        <FontAwesomeIcon icon={faCompressArrowsAlt} className="text-sm" />
                    </button>
                    <button onClick={() => setDisplaySidebar(false)} title="隱藏" className="hover:text-rose-500 transition-colors">
            ...
            <div className="flex items-center gap-2 text-xs font-bold text-gray-400 mb-2 px-1">
                {curSelectedSection ? (
                    <div className="flex items-center gap-1 cursor-pointer hover:text-indigo-600 transition-colors" onClick={() => {
                        const newSelected = [...selectedSection]
                        newSelected[category] = null
                        setSelectedSection(newSelected)
                    }}>
                        <span>{curCategory.title}</span>
                        <FontAwesomeIcon icon={faAngleRight} className="text-[8px]" />
                        <span className="text-gray-800">{curCategory.sections.find(s => s.id === curSelectedSection)?.title}</span>
                    </div>
                ) : (
                    <span className="text-gray-800">{curCategory.title}</span>
                )}
            </div>
            ...
            {curSelectedSection ? (
                <SidebarSectionComponent 
                    section={curCategory.sections.find((sec) => sec.id === curSelectedSection)!}
                    folding={{
                        isFolded: (sid, gid) => isFolded(category, sid, gid),
                        toggle: (sid, gid) => toggleFolded(category, sid, gid)
                    }}
                    activeArticle={activeArticle}
                />
            ) : (
